...
app.layout = html.Div(style={'display': 'flex', 'height': '100vh', 'padding': '10px'}, children=[
    ...
    html.Div(style={'width': '18%', 'paddingRight': '10px'}, children=[
        ...
        html.Br(),
        html.P("Select Clusters to Show:", style={'fontSize': '12px', 'marginTop': '10px'}),
        dcc.Checklist(
            id='cluster-filter',
            options=[],  # This will be dynamically populated
            value=[],    # Initially, no clusters are selected
            style={'fontSize': '12px'},
            inline=True,
        ),
    ]),
...
@app.callback(
    ...
    Output('cluster-filter', 'options'),  # Output to dynamically update cluster options
    Input('range-slider', 'value'),
    Input('cluster-toggle', 'value'),
    Input('cluster-count', 'value'),
    Input('cluster-filter', 'value')  # Get selected clusters
)
def update_figure(selected_range, cluster_toggle, k, selected_clusters):
    ...
    cluster_options = []  # List to store cluster options for filtering
    ...
    if 'enable' in cluster_toggle and not filtered_df.empty:
        ...
        if len(valid) >= k:
            # Perform clustering only based on spatial coordinates
            ...
            # Create list of cluster options for the filter checklist
            cluster_options = [{'label': f'Cluster {i}', 'value': i} for i in range(k)]
    ...
    return fig_main, fig_cluster, fig_timeline, cluster_options
